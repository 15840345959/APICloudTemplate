...
(function( window, undefined ) {
    ...
    auiPopup.prototype = {
        _init: function() {
        	...
        	console.log("---------------"+JSON.stringify(_btn))
        	if(_btn){

        		for(var i=0;i<_btn.length;i++){
                    console.log("-----_btn--------"+JSON.stringify(_btn[i]))
        			_btn[i].setAttribute("tapmode", "");
        			_btn[i].onclick = function(e){
                        console.log("-----_btn111111111111")
        				var popupId = this.getAttribute(CLASS_POPUP_FOR);
        				var popupDom = document.getElementById(popupId);

                        console.log("-----popupId--------"+JSON.stringify(popupId))
                        console.log("-----popupDom--------"+JSON.stringify(popupDom))
        				if(popupDom){
